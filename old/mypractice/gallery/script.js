...
function addPhotos(){
	...
	var nav = [];
	for(s in data){
		var _html = template.replace("{{index}}",s).replace("{{img}}",data[s].img).replace("{{desc}}",data[s].desc);
		html.push(_html);
...
function rsort(n){
	...
	for(s=0;s<_photo.length;s++){
		...
		_photo[s].style.left='';
		_photo[s].style.top='';
		_photo[s].style['transform']=_photo[s].style['-webkit-transform']='rotate(360deg)  scale(1.2)';
	...
	g('#bgi').style.backgroundImage = 'url(img/'+n+'.jpg)';
